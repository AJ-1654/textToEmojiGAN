...
from utils.dataset_utils import load_dataset as load_dataset_gan1
...
def load_dataset_gan(img_path, txt_path, img_shape=(64, 64, 3), split_rate=0.1):
	...
	d = np.append(image_train, image_test, axis=0)
	p = np.append(caption_train, caption_test)
...
def inception_score(x, batch_size=26):
    ...
    e = np.sum(e, axis=1) # KL(x) = ÎŁ_y p(y|x)log(P(y|x)/P(y))
    e = np.mean(e, axis=0)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        # emoji gan
        x_train, y_train, _, _, _ = load_dataset_gan1(img_path, txt_path, img_size, split_rate=0.00000000001)
        print(x_train.shape)
        print(y_train.shape)
